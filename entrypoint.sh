...
if [ -f "$MOUNTED_CONFIG_MARKER" ] && [ "$FORCE_CONFIG_GENERATION" != "true" ]; then
  ...
  envsubst '${PROXY_PORT} ${WORKER_PROCESSES} ${WORKER_CONNECTIONS} ${RESOLVER} ${RESOLVER_IPV6} ${RESOLVER_TIMEOUT} ${PROXY_CONNECT_TIMEOUT} ${PROXY_SEND_TIMEOUT} ${PROXY_READ_TIMEOUT} ${SEND_TIMEOUT} ${CLIENT_BODY_TIMEOUT} ${CLIENT_HEADER_TIMEOUT} ${KEEPALIVE_TIMEOUT}' < "$TEMPLATE_FILE" > "$CONFIG_FILE"
fi
