...
MOUNTED_CONFIG_MARKER="/usr/local/nginx/conf/.config_mounted"

# Check if a custom config was mounted (indicated by marker file)
# Only skip template generation if marker exists AND FORCE_CONFIG_GENERATION is not true
if [ -f "$MOUNTED_CONFIG_MARKER" ] && [ "$FORCE_CONFIG_GENERATION" != "true" ]; then
  echo "Found mounted nginx.conf, skipping template generation"
  ...
  envsubst '${PROXY_PORT} ${WORKER_PROCESSES} ${WORKER_CONNECTIONS} ${RESOLVER} ${RESOLVER_IPV6} ${RESOLVER_TIMEOUT} ${PROXY_CONNECT_TIMEOUT} ${PROXY_HTTP_VERSION} ${PROXY_SEND_TIMEOUT} ${PROXY_READ_TIMEOUT} ${SEND_TIMEOUT} ${CLIENT_BODY_TIMEOUT} ${CLIENT_HEADER_TIMEOUT} ${KEEPALIVE_TIMEOUT}' < "$TEMPLATE_FILE" > "$CONFIG_FILE"
fi
